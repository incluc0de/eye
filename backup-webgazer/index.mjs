...
console.log(
  '### INDEX.MJS MODIFICADO CARREGADO ###'
);
...
async function loop() {
  ...
  if (!paused) {
    ...
    console.log(
      '[WebGazer] loop ativo - iniciando frame'
    );

    // Paint the latest video frame into the canvas
    paintCurrentFrame(
      videoElementCanvas,
    ...
    console.log(
      '[WebGazer] frame pintado',
      {
        canvasWidth: videoElementCanvas.width,
        canvasHeight: videoElementCanvas.height,
    ...
    console.log(
      '[WebGazer] chamando getPrediction()'
    );

    latestGazeData = getPrediction();

    console.log(
      '[WebGazer] getPrediction() retornou Promise'
    );
    ...
    // Feedback box
    if (
      webgazer.params.showFaceFeedbackBox
    ) {
      checkEyesInValidationBox();
    }

    console.log(
      '[WebGazer] aguardando getPrediction()...'
    );

    latestGazeData =
      await latestGazeData;

    console.log(
      '[WebGazer] resultado getPrediction():',
      latestGazeData
    );

    // Callback registered by setGazeListener
    callback(
      latestGazeData,
      elapsedTime
    );
    ...
    if (latestGazeData) {
      ...
      var len =
        smoothingVals.length;

      for (
        var d = 0;
        d < smoothingVals.length;
        d++
      ) {

        x +=
          smoothingVals.get(d).x;

        y +=
          smoothingVals.get(d).y;
      }
...
async function init(stream) {
  ...
  const videoPreviewSetup = new Promise((res) => {
    ...
    function setupPreviewVideo() {
      ...
      // Ainda não temos dimensões válidas do vídeo
      if (
        !videoElement.videoWidth ||
        !videoElement.videoHeight
      ) {
      ...
      console.log(
        '[WebGazer] Configurando preview do vídeo',
        {
      ...
      // Define o tamanho do buffer interno
      setInternalVideoBufferSizes(
        videoElement.videoWidth,
        videoElement.videoHeight
      );
  
      // Define o tamanho visual do preview
      ...
      videoContainerElement.appendChild(
        videoElementCanvas
      );
      ...
      // Remove o listener, caso tenha sido registrado
    ...
    console.log(
      '[WebGazer] Estado inicial do vídeo:',
      {
        readyState: videoElement.readyState,
        videoWidth: videoElement.videoWidth,
        videoHeight: videoElement.videoHeight
     ...
     *
     * Se loadeddata já aconteceu, não esperamos
     * por um evento que não ocorrerá novamente.
     */
    if (
      ...
      console.log(
        '[WebGazer] Vídeo já possui dados. Configurando imediatamente.'
      );
      ...
      console.log(
        '[WebGazer] Aguardando evento loadeddata...'
      );
